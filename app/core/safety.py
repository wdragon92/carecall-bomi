...
# (2) 구절을 '…다는 생각/말' 식으로 받아 없/안/못으로 부정하는 명사화 부정.
#     무관한 뒷절 부정과 구분하려 창을 좁게(9자) 두고, 명사화 표지와 부정어의 동시 출현만 본다.
_NEG_NOMINAL = ("다는", "단 ")
...
RULES: tuple[Rule, ...] = (
    ...
    Rule("medical_soon", "건강", "보통", False, "신경 쓰이는 건강 신호 — 진료 상담 권고",
         ("살이 빠", "몸무게가 줄", "며칠째 열", "열이 안 내려", "변이 검", "변에 피",
          "혈변", "다리가 부", "종아리가 붓")),
    # 편측 저림·심장 방사통 — 뇌졸중/심장 확인. 단독 저림은 자세성 등 양성 원인도 많아
    # '응급(119)'까진 아니되 방치 금물(빠른 진료). 힘 빠짐·발음 이상 동반은 위 응급 규칙이 잡는다.
    Rule("medical_soon", "건강", "높음", False, "저림·방사통 신호 — 뇌졸중·심장 확인 위해 빠른 진료 권고",
         ("한쪽이 저리", "한쪽이 저려", "한쪽 팔이 저리", "한쪽 팔이 저려",
          "왼팔이 저리", "왼팔이 저려", "오른팔이 저리", "오른팔이 저려",
          "손발이 저리", "손발이 저려", "턱이 아프고 팔")),
    # ── 자살·자해 즉각 위험 (P0) ──
    Rule("suicide_acute", "긴급", "높음", True, "자살·자해 즉각 위험 신호 — 즉시 109/119·보호자 연결",
         ("죽어버", "죽으려", "죽고 싶", "확 죽", "목맬", "뛰어내", "약을 모아",
          "손목을 그", "자는 듯이 갔으면", "이제 못 볼",
          # 동음이의 오탐 방지: 위험 어휘는 '실행' 문맥과 결합해서만 발동
          # ("농약 쳤다"·"번개탄 샀다"·"유서 깊은"·모임에서 "먼저 간다" 같은 일상어 제외)
          "목을 매고 싶", "목을 매려", "목을 매서", "목을 맬", "목을 매달",
          "농약 마시", "농약 마셔", "농약 마셨", "농약을 마시", "농약을 마셔", "농약을 마셨",
          "농약 먹", "농약을 먹", "농약 들이",
          "번개탄 피", "번개탄을 피",
          "유서 남기", "유서를 남기", "유서 쓰", "유서를 쓰", "유서 써", "유서를 써",
    ...
    Rule("suicide_warning", "정서", "높음", True, "삶에 대한 절망/체념 신호 — 따뜻한 확인 및 상담 연결 권고",
         ("죽을 때가 됐", "그만 살", "살아서 뭐", "이만하면 됐", "오래 살아 뭐", "없어져도",
          ...
          # 미탐 보강: 완곡한 죽음 소망·체념 표현 (과탐 피하려 '더 나음'·'가고 싶음' 형태로 한정)
          "죽는 게 나아", "죽는 게 나은", "죽는 게 낫",
          "저세상 가고 싶", "하늘나라 가고 싶", "하늘나라 갔으면",
          "눈 감고 싶", "눈을 감고 싶", "사라지고 싶", "다 끝내고 싶")),
    ...
    Rule("fraud_exposure", "사기_노출", "높음", True, "사기 의심 정황 언급 — 송금 중단·112/1332 안내",
         ("보이스피싱", "스미싱", "사기 같", "사기인 것 같", "사기당", "사기를 당",
          ...
          # 이미 실행했다고 알리는 완료형 — 결정망이 놓치면 리포트에서 사라짐(실측).
          # 가족 송금·요리("전 부쳤")·노래("불러줬") 오발을 피하려 외부 지시·개인정보 노출 문맥으로 한정
          "링크를 눌렀", "링크 눌렀", "문자를 눌렀",
          "비밀번호를 불러줬", "비밀번호 불러줬", "계좌번호를 불러줬", "계좌번호 불러줬",
          "불러준 계좌로", "불러준 계좌에", "알려준 계좌로", "알려준 계좌에",
          "시키는 대로 송금", "시키는 대로 이체", "시키는 대로 부쳤")),
)
...
def _negated(text: str, start: int, end: int) -> bool:
    ...
    if any(n in tail for n in _NEG_TAIL):
        return True
    # 명사화 부정: "~다는 생각은 없/안/못" (좁은 창에서 표지+부정어 동시 출현만)
    if any(nm in tail for nm in _NEG_NOMINAL) and any(n in tail for n in ("없", "안", "못", "아니")):
        return True
    return False
