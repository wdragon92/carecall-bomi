...
def test_scan_negation_precise_direct_only():
    ...
    # (c) 명사화 '존재 부정'(생각/마음 + 없/안 해) — 인용형·관형형 둘 다 정확히 미매칭
    assert "suicide_acute" not in _kinds("어르신: 죽고 싶다는 생각은 안 해요")   # 인용형
    assert "suicide_acute" not in _kinds("어르신: 죽고 싶다는 생각은 없어요")
    assert "suicide_acute" not in _kinds("어르신: 죽고 싶은 생각은 없어")        # 관형형(-은)
    assert "suicide_acute" not in _kinds("어르신: 죽고 싶은 마음은 전혀 없어")
    # (d) 표현명사(말/소리) + '못 하다'는 실제 SI 고백일 수 있어 탐지 유지(FN 방지)
    assert "suicide_acute" in _kinds("어르신: 죽고 싶다는 말도 못 하고 끙끙 앓았어")
    # 참-양성은 그대로 유지
    assert "suicide_acute" in _kinds("어르신: 요즘 죽고 싶다는 생각이 자꾸 들어요")


def test_scan_method_words_bare_gated_homonyms():
    # 안전-우선: 치명 method 어휘(농약·번개탄·목을 매)는 넓게 잡아 미탐 방지(오탐 감수).
    # 자살 안전망에서 '방법 준비' 발화를 놓치는 FN은 일상어 오탐보다 훨씬 치명적이다.
    assert "suicide_acute" in _kinds("어르신: 농약 사놨어")
    assert "suicide_acute" in _kinds("어르신: 번개탄 준비했어")
    assert "suicide_acute" in _kinds("어르신: 유서 썼어")
    assert "suicide_acute" in _kinds("어르신: 유서를 남겼어")
    assert "suicide_acute" in _kinds("어르신: 목을 매려고 했어")
    assert "suicide_acute" in _kinds("어르신: 농약을 마셨어")
    # 오탐이 매우 흔한 표층만 게이팅 유지 — 발동하지 않음
    assert "suicide_acute" not in _kinds("어르신: 이 동네는 유서 깊은 곳이야")  # 유서=자살 활용형만
    assert "suicide_acute" not in _kinds("어르신: 그럼 나 먼저 간다")           # 통화 종료 인사
    assert "medical_emergency" not in _kinds("어르신: 긴장돼서 식은땀 났어")     # 식은땀=심장문맥만
...
def test_scan_s13_passive_suicide_additions():
    ...
    # 일상어 오발이 커 제거된 문구는 이제 발동 안 함(경보 없음)
    for benign in ["일을 다 끝내고 싶어", "피곤해서 눈 감고 싶어", "창피해서 사라지고 싶어"]:
        assert "suicide_warning" not in _kinds("어르신: " + benign), benign
...
def test_scan_fraud_completed_forms_removed():
    # S13 완료형 사기 문구는 가족송금·카톡링크 오발이 커 제거 — 이제 발동 안 함.
    # (완료형 송금 대응은 conversation._FRAUD_SENT 행동카드가 담당, LLM 추출이 보완)
    for benign in ["며느리가 시키는 대로 송금했어", "카톡 링크를 눌렀어",
                   "은행 직원이 불러준 계좌로 넣었어", "손자한테 용돈 송금했어",
                   "명절이라 전 부쳤어", "손주한테 노래 불러줬어"]:
        assert "fraud_exposure" not in _kinds("어르신: " + benign), benign
    # 기존 fraud 규칙(지시형·기관사칭)은 그대로 발동
    assert "fraud_exposure" in _kinds("어르신: 검찰이라면서 돈을 보내라는 문자가 왔어")
    assert "fraud_exposure" in _kinds("어르신: 앱을 깔라고 하던데 괜찮을까")


def test_scan_cardiac_radiation_is_medical_soon():
    # S13: 심장 방사통(아픔이 팔·턱으로 뻗침) — 빠른 진료(warning)
    for utter in ["턱이 아프고 팔이 저려", "가슴이 아프고 팔이 저려", "어깨가 아프고 가슴이 답답"]:
        k = _kinds("어르신: " + utter)
        assert "medical_soon" in k, utter
        assert safety.alert(k)[0] == "warning"
    # 단독·양측 저림은 자세성 오발이 커 제외(발동 안 함) — 편측 마비·힘빠짐은 응급 규칙이 별도로
    assert "medical_soon" not in _kinds("어르신: 자고 일어났더니 손발이 저려")
