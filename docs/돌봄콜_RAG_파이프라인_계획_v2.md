...
| 핵심 기능·기술 구현도 (35) | AI(CLOVA·Gemini) / API(공공데이터포털 2종 + CLOVA) / DB(SQLite: 세션·해시·인덱스 메타) / 서버(NCP·FastAPI) — 명시된 4요소 전부 |
...
| 계층 | 사용 기술 | 비고 |
|---|---|---|
| AI | CLOVA Studio (STT·TTS·OCR·챗·임베딩) + Gemini(A/B 비교) | 과정 핵심 |
| 검색 | **FAISS on NCP 서버** | 라이브러리(≠외부 SaaS). 강사 교안 조합 그대로. 데이터가 서버 밖으로 안 나감 = 소버린 부합 |
| 저장 | Object Storage(인덱스·해시 백업) / SQLite 또는 Cloud DB(세션·리포트) | 루브릭 "DB" 명시 대응 |
| 서버 | NCP 서버 + FastAPI | |
...
| **P4** | 평가셋 15개(카드에서 LLM 자동생성+문서밖 3개) + Hit@4·거부율 + 데모 리허설 | 승용+CC | 반나절 |
...
- [x] Gemini 키 (aistudio.google.com) — 과정에서 발급 완료. 백엔드 A/B 비교용
